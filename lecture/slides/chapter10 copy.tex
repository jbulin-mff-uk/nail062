...
Na druhou stranu v ní lze dokázat všechna \alert{existenční tvrzení o numerálech}, která jsou pravdivá v $\underline{\mathbb N}$. Tím myslíme formule, které v prenexním tvaru mají pouze existenční kvantifikátory, a do kterých jsme za volné proměnné substituovali \alert{numerály} $\underline{n}=S(\dots S(0)\dots)$. 
...
Užitečným rozšířením Robinsonovy aritmetiky je tzv. Peanova aritmetika, ve které lze \alert{dokazovat indukcí}:

\begin{definition}[Peanova aritmetika]
\alert{Peanova aritmetika} $\text{\it PA}$ je extenze Robinsonovy  aritmetiky $Q$ o \alert{schéma indukce}, tj. pro každou $L$-formuli $\varphi(x,\overline{y})$ přidáme následující axiom:
$$
...
\begin{remark}
Pokud bychom se přesunuli do logiky \alert{2. řádu}, potom bychom už mohli strukturu $\underline{\mathbb N}$ axiomatizovat (až na izomorfismus), a to extenzí Peanovy aritmetiky o následující formuli 2. řádu, tzv. \alert{axiom indukce}:
$$
