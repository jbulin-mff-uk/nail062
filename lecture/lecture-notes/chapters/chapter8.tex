...
% \begin{frame}[fragile]{Program v Prologu}
    ...
    \begin{tabular}{lr}
       ...
       \emph{$\{son(X,Y),\neg father(Y,X),\neg man(X)\}$} 
        ...
        father(bob,charlie). 
        &
        \emph{$\{father(bob,charlie)\}$}
        \\
        mother(alice,charlie). 
        &
        \emph{$\{mother(alice,charlie)\}$}
        \\
        \\
        ?-son(charlie,X). 
        &
...
% \begin{frame}{LI-rezoluce v Prologu}
    ...
    Platí v programu daný \emph{existenční dotaz},  \textcolor{blue}{$P\models(\exists X)son(charlie,X)$}?
    ...
    Je-li odpověď na dotaz kladná, chceme znát i \emph{výstupní substituci} $\sigma$, tj. složení unifikací z rez. kroků, zúženné na proměnné v $G$. Platí:
    $$
...
% \begin{frame}{Příklady}
    ...
    \texttt{X=bob} \qquad výstupní substituce \textcolor{blue}{$\sigma=\{X/b\}$}
    ...
    \texttt{X=alice} \qquad výstupní substituce \textcolor{blue}{$\sigma=\{X/a\}$}
