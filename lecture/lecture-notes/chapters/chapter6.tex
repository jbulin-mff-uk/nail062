...
\begin{itemize}
    ...
    \item The values of terms are evaluated according to their trees, where symbols are replaced by their interpretations (functions, and constants from the domain),
    \item From the values of terms, we obtain the truth values of atomic formulas: is the resulting $n$-tuple in the relation (interpreting the given relation symbol)?
...
\begin{definition}[Definable Sets]
    ...
    $$
    For brevity, we can also write $\varphi^\A(\bar x)=\{\bar a\in A^n\mid\A\models\varphi[e(\bar x/\bar a)]\}$. A set $R\subseteq A^n$ is \emph{definable in the structure $\A$} if there exists a formula $\varphi(\bar x)$ such that $R=\varphi^\A(\bar x)$.
\end{definition}
