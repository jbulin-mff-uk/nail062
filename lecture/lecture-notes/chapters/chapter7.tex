...
\begin{definition}[Congruence]
    ...
    \end{itemize}    
    \emph{A congruence of the structure} $\A$ is an equivalence $\sim$ on the set $A$ that is a congruence for all functions and relations of $\A$. We denote the equivalence class of an element $a\in A$ by $[a]_\sim=\{b\in A\mid a\sim b\}$.
\end{definition}
...
\begin{definition}[Axioms of Equality]
    ...
    \begin{enumerate}[(i)]
        \item $x=x$,
        \item $x_1=y_1\land\cdots\land x_n=y_n\limplies f(x_1,\dots,x_n)=f(y_1,\dots,y_n)$ for every $n$-ary (non-constant) function symbol $f$ of the language $L$,
...
\begin{example}
For illustration, consider the ring of integers $\underline{\mathbb Z}$, where the symbol ``$=$'' is interpreted as $\equiv_2$ (congruence modulo $2$), and its quotient structure $\underline{\mathbb Z}/_{\equiv_2} \simeq \underline{\mathbb Z_2}$, where ``$=$'' is interpreted as the identity. As a formula $\varphi$, take for example $x = y \limplies x + 1 = y + 1$.
\end{example}
