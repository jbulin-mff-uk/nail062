...
\begin{remark}
    Ve zbytku kapitoly se budeme zabývat jen jazyky bez rovnosti. Pro jazyky s rovností by stačilo přidat do $T$ axiomy rovnosti, resp. do CNF formule $S$ jejich CNF ekvivalenty, a v důkazu úplnosti níže použít variantu Herbrandovy věty pro jazyk s rovností (viz Poznámka \ref{remark:herbrand-theorem-equality}). V praxi je ale efektivnější použít speciální inferenční pravidla pro rovnost, např. tzv. \emph{paramodulaci}, která umožňuje rovnou přepsat podterm na jemu rovný. Detaily zde neuvedeme.
\end{remark}
...
Mějme klauzule $C_1$ a $C_2$ s disjunktními množinami proměnných. Jsou-li $C^*_1$ a $C^*_2$ základní instance klauzulí $C_1$ a $C_2$ a je-li $C^*$ rezolventou $C^*_1$ a $C^*_2$, potom existuje rezolventa $C$ klauzulí $C_1$ a $C_2$ taková, že $C^*$ je základní instancí $C$.
\end{lemma}
\begin{proof}
