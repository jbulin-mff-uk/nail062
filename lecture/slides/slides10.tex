...
\begin{frame}{Rezoluční pravidlo}
    ...
    Disjunktní množiny proměnných lze získat přejmenováním. Proč? Např. z \myexampleinline{
        $\{\{P(x)\},\{\neg P(f(x))\}\}$
    } lze získat $\square$, nahradíme-li $\{P(x)\}$ klauzulí $\{P(y)\}$. Ale $S=\{P(x),P(f(x))\}$ není unifikovatelná.
...
\begin{frame}{Rezoluční důkaz}
    ...
    Proč potřebujeme odstranit více literálů z jedné klauzule najednou? Např. \myexampleinline{
        $S=\{\{P(x),P(y)\},\{\neg P(x),\neg P(y)\}\}$
    } je zamítnutelná, ale neexistuje zamítnutí, které by v každém kroku odstranilo jen jeden.  
...
\begin{frame}{Příklad rezolučního stromu}
