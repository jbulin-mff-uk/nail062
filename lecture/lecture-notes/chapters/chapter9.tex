...
\begin{proposition}
    ...
    $$
\end{proposition}
The idea of the proof is based on the fact that an element $a \in A$ can be mapped to an element $b \in B$ if and only if they have the ``same properties,'' that is, if they satisfy exactly the same formulas with one free variable $\varphi(x)$. However, due to the finiteness of $A$, there are only finitely many ``properties of elements,'' at most $2^{|A|}$. Indeed, we have $\mathcal A \models \varphi(x) \iff \psi(x)$ precisely when $\varphi(x)$ and $\psi(x)$ define the same subset of elements of $A$. Hence, one can write a single sentence stating ``There exists an element with all the properties of the element $a$.'' This sentence must also be valid in $\mathcal B$.
\begin{proof}
    ...
    Let us also mention why the mapping defined in this way is a bijection; for this it suffices to show that it is injective. Let $a\neq a'\in A$, and assume that $h(a)$ was defined earlier. Then $\neg x = c_a$ was one of the properties of the element $a'$, that is, it had to hold also for $h(a') = b' \in B$. Thus we have $h(a') = b' \neq c_a^{\mathcal B'} = b = h(a)$.
\end{proof}
...
\end{align*}
Here, the first equivalence follows from the definition of a definable set, the second from the properties of isomorphisms (Proposition~\ref{proposition:isomorphism-preserves-semantics}), the third expands the composition of functions (where $e \circ h$ means that we apply $e$ first and then $h$), in the fourth we use the assumption that $h$ fixes the parameters $\overline{b}$, and the fifth is again the definition of a definable set.
\end{proof}
...
\begin{definition}[Isomorphism Spectrum, $\kappa$-categoricity]
    ...
    I(\kappa,T)$ of models of $T$ of cardinality $\kappa$ up to isomorphism, for each cardinality $\kappa$ (including \emph{transfinite} ones\footnote{Informally, different infinite ``sizes'' of domains.}). A theory $T$ is \emph{$\kappa$-categorical} if $
    I(\kappa,T)=1$.
\end{definition}
