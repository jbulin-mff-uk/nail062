...
\begin{proof}
Nechť $\sigma=\{x_1/t_1,\dots,x_n/t_n\}$ a $\tau=\{y_1/s_1,\dots,y_m/s_m\}$. Stačí dokázat v případě, kdy výraz $E$ je jediná proměnná, zbytek snadno plyne indukcí. (Substituce nijak nemění ostatní symboly.)

Je-li $E=x_i$ pro nějaké $i$, potom $E\sigma=t_i$ a $(E\sigma)\tau=t_i\tau=E(\sigma\tau)$.

\todo
% Je-li $v$ proměnná $x_i$ pro nějaké $i$, je \mygreen{$v\sigma=t_i$} a \mygreen{$(v\sigma)\tau=t_i\tau$}, což je $v(\sigma\tau)$
...
Tím jsme dokázali (i). Asociativitu (ii) nyní snadno dokážeme opakovaným užitím (i). Následující platí pro každý výraz $E$, tedy i pro $E\in\{x_1,\dots,x_n\}$:
$$
