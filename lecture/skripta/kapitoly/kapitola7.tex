...
\begin{definition}[Kongruence]
    ...
    \end{itemize}    
    \emph{Kongruence struktury} $\A$ je ekvivalence $\sim$ na množině $A$, která je kongruencí pro všechny funkce a relace $\A$. Třídu prvku $a\in A$ podle $\sim$ značíme $[a]_\sim=\{b\in A\mid a\sim b\}$.
\end{definition}
...
\begin{definition}[Axiomy rovnosti]
    ...
    \begin{enumerate}[(i)]
        \item $x=x$,
        \item $x_1=y_1\land\cdots\land x_n=y_n\limplies f(x_1,\dots,x_n)=f(y_1,\dots,y_n)$ pro každý $n$-ární (nekonstantní) funkční symbol $f$ jazyka $L$,
...
\begin{example}
    Pro ilustraci, uvažte okruh celých čísel $\underline{\mathbb Z}$, kde symbol ``='' je interpretován jako $\equiv_2$ (kongruence modulo $2$), a jeho faktorstrukturu $\underline{\mathbb Z}/_{\equiv_2}\simeq \underline{\mathbb Z_2}$, kde ``='' je interpretován jako identita. Jako formuli $\varphi$ vezměte např. $x=y\limplies x+1=y+1$.
\end{example}
