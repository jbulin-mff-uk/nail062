...
\end{proposition}
\begin{proof}
    Je-li $h$ izomorfismus, vlastnosti snadno dokážeme indukcí podle struktur termu resp. formule. Naopak, je-li $h$ bijekce splňující (i) a (ii), dosazením $t=f(x_1,\dots,x_n)$ resp. $\varphi=R(x_1,\dots,x_n)$ dostáváme vlastnosti z definice izomorfismu.
\end{proof}
...
%\todo example computing Aut(G) for a graph G
...
\end{cases}
$$
Spočetné modely až na izormofismus jsou například:
$$ 
...
Z věty o kompaktnosti snadno získáme následující tvrzení, pomocí kterého lze ukázat neaxiomatizovalnost např. konečných grafů, konečných grup, konečných těles.
...
\begin{theorem}\label{theorem:finite-axiomatizability}[O konečné axiomatizovatelnosti]
