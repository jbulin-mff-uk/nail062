...
\begin{example}
    Let us compute the resolution closure of the CNF formula 
    $$
    S=\{\{p,\neg q,r\},\{p,\neg r\},\{\neg p,r\},\{p,s\},\{q,r\}\}.
    $$ 
    The clauses from $S$ are in \textcolor{blue}{blue}; additional clauses are obtained by resolving them in pairs (first with the first, second with the first, second with the second, etc., over all possible literals):
    \begin{align*}
