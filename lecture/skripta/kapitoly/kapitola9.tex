...
\begin{proposition}\label{proposition:isomorphism-preserves-semantics}
...
\begin{proposition}
    ...
    $$
\end{proposition}
Myšlenka důkazu je založena na tom, že prvek $a\in A$ lze zobrazit na prvek $b\in B$ právě tehdy, když mají `stejné vlastnosti', tj. platí o nich tytéž formule s jednou volnou proměnnou $\varphi(x)$. Díky konečnosti $A$ máme ale jen konečně mnoho `vlastností prvků', nejvýše $2^{|A|}$. Platí totiž $\A\models\varphi(x)\liff\psi(x)$ právě když $\varphi(x)$ a $\psi(x)$ definují stejnou podmnožinu prvků $A$. Lze tedy napsat jedinou sentenci, která říká ``Existuje prvek se všemi vlasnostmi prvku $a$''. Tato sentence musí platit i v $\B$.
\begin{proof}
    ...
    Zmiňme ještě, proč je takto definované zobrazení bijekce, k tomu stačí, že je prosté. Buď $a\neq a'\in A$, a nechť $h(a)$ jsme definovali dříve. Potom $\neg x=c_a$ byla jedna z vlasností prvku $a'$, tj. musela platit i o $h(a')=b'\in B$, a máme $h(a')=b'\neq c_a^{\B'}=b=h(a)$.
\end{proof}
...
\end{align*}
Zde první ekvivalence plyne z definice definovatelné množiny, druhá z vlastností izomorfismu (Tvrzení \ref{proposition:isomorphism-preserves-semantics}), třetí rozepisuje skládání funkcí (kde $e\circ h$ znamená, že nejprve aplikujeme $e$ a potom $h$), ve čtvrté používáme předpoklad, že $h$ fixuje parametry $\overline{b}$, a pátá je opět definice definovatelné množiny.
\end{proof}
...
\begin{definition}[Izomorfní spektrum, $\kappa$-kategoricita]
    ...
    I(\kappa,T)$ modelů $T$ kardinality $\kappa$ až na izomorfismus, pro každou kardinalitu $\kappa$ (včetně \emph{transfinitních}\footnote{Neformálně řečeno, různé nekonečné ``velikosti'' domén.}). Teorie $T$ je \emph{$\kappa$-kategorická}, pokud $I(\kappa,T)=1$.
\end{definition}
...
Na závěr této kapitoly se podíváme, za jakých okolností lze `popsat' (\emph{axiomatizovat}) třídu modelů respektive teorii. Zajímat nás bude také kdy si vystačíme s konečně mnoha axiomy, a kdy to lze pomocí otevřených axiomů (kterých může být i nekonečně mnoho). Srovnejte tuto otázku s Tvrzením \ref{proposition:axiomatize-in-DNF-CNF} z výrokové logiky. 
